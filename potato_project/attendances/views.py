...
class AttendanceViewSet(viewsets.ViewSet):
    ...
    def increment(self, request):
        ...
        new_attendance = Attendance.objects.create(user_id=user, date=today, coin_awarded=1)
    ...
    def decrement(self, request):
        ...
        if attendance.coin_awarded < decrement_value:
            return Response({"message": "코인이 모자라요!"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # 성공 응답 반환
        return Response({"message": "물건을 구매했습니다.", "coin_awarded": attendance.coin_awarded})

...
class AttendanceViewSetTestCase(APITestCase):
    ...
    def test_list_attendance_with_record(self):
        Attendance.objects.create(
            user_id=self.user, date=timezone.now().date(), coin_awarded=1
        )
        response = self.client.get(self.attendance_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        attendance = (
            Attendance.objects.filter(user_id=self.user).order_by("-date").first()
        )
    ...
    def test_decrement_attendance_success(self):
        Attendance.objects.create(
            user_id=self.user, date=timezone.now().date(), coin_awarded=5
        )

...
class MonthlyCompletionRateView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        daily_rates = Todo.objects.filter(
            ...
            total=Count('id'),
            completed=Count(Case(When(is_done=True, then=1), output_field=IntegerField()))
        ).order_by('date__date')

        rates = {str(item['date__date']): round(item['completed'] / item['total'] * 100, 2) if item['total'] > 0 else 0 

...
class TodoAPITestCase(TestCase):
    def setUp(self):
        ...
        self.todo = Todo.objects.create(user_id=self.user, task='Test Task', is_done=False, date=timezone.now())
    ...
    def test_create_todo(self):
        ...
        response = self.client.post(reverse('todo-list-create'), data=json.dumps(data), content_type='application/json')
    ...
    def test_update_todo(self):
        ...
        response = self.client.put(reverse('todo-detail', args=[self.todo.id]), data=json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 200)
